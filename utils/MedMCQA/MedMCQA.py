...
class MedMCQA(BaseDataset):
    def __init__(self,model,dataset_path,output_path):
        ...
        self.split = os.environ.get("MEDMCQA_SPLIT", "test")
        self.samples = []
    ...
    def construct_messages(self,sample):
        ...
        cop = sample.get("cop", None)
        if isinstance(cop, int) and 0 <= cop < len(alphas):
            sample["answer"] = alphas[cop]
        else:
            sample["answer"] = None
        return sample
    ...
    def cal_metrics(self,out_samples):
        ...
        metrics = {
            "total metrics":{"total":total,"right":right,"acc":acc},
